...
root='/mnt/sdb/ocr/train/task1_train/*.[j|p]*'  # ICDAR2019数据
jpgPath = glob(root)
...
textModelPath = './train/models/weights_densenet_ICDAR2019-10-0.92.h5'
if os.path.exists(textModelPath):
    print("Loading textModelPath weights...")
    textModel.load_weights(textModelPath, by_name=True, skip_mismatch=True)
    print('done!')


checkpoint = ModelCheckpoint(filepath='./train/models/weights_densenet_ICDAR2019-{epoch:02d}-{val_confidence_loss_loss:.2f}.h5',
                             monitor='confidence_loss_loss', save_best_only=False, save_weights_only=True)
tensorboard = TensorBoard(log_dir='./train/models/ICDAR2019_logs', write_graph=True)
